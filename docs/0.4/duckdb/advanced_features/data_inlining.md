...
If a table has a [sort order defined]({% link docs/0.4/duckdb/advanced_features/sorted_tables.md %}), `ducklake_flush_inlined_data` sorts the output by those keys before writing the resulting Parquet file. The sort order applied is the one currently active on the table at the time of the flush.
