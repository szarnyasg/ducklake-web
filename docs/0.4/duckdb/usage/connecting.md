...
In the `ATTACH` command, you must specify the [catalog database]({% link docs/0.4/duckdb/usage/choosing_a_catalog_database.md %}) and the [data storage location]({% link docs/0.4/duckdb/usage/choosing_storage.md %}).
...
| `DATA_INLINING_ROW_LIMIT`                          | The number of rows for which [data inlining]({% link docs/0.4/duckdb/advanced_features/data_inlining.md %}) is used  | `0`                                                                                     |
