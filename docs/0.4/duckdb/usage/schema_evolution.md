...
The [types]({% link docs/0.4/specification/data_types.md %}) of columns can be changed.
...
Columns are tracked using **field identifiers**. These identifiers are stored in the `column_id` field of the [`ducklake_column` table]({% link docs/0.4/specification/tables/ducklake_column.md %}).
...
When reading the data for a table, the schema together with the correct field identifiers is read from the [`ducklake_column` table]({% link docs/0.4/specification/tables/ducklake_column.md %}).
...
- Any column that has a `field_id` that does not exist in the data file must be replaced with the `initial_default` value in the [`ducklake_column` table]({% link docs/0.4/specification/tables/ducklake_column.md %})
