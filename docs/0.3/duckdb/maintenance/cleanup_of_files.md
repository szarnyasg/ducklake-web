...
When files are no longer required in DuckLake, due to e.g. [snapshots being expired]({% link docs/0.3/duckdb/maintenance/expire_snapshots.md %}) or [files being merged]({% link docs/0.3/duckdb/maintenance/merge_adjacent_files.md %}), they are not immediately deleted.
The reason for this is that there might still be active queries that are scanning these files.

The files are instead added to the [`ducklake_files_scheduled_for_deletion` table]({% link docs/0.3/specification/tables/ducklake_files_scheduled_for_deletion.md %}).
