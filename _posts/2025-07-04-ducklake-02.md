...
We would like to start with a quick reminder: the term [DuckLake]({% link faq.md %}#what-is-ducklake) refers to both the [DuckLake open standard]({% link docs/stable/specification/introduction.md %}) and the [`ducklake` DuckDB extension](https://duckdb.org/docs/current/core_extensions/ducklake).
