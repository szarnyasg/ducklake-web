...
def replace_jekyll_tags_for_variables(doc_body, config):
    doc_body = doc_body.replace("{{ site.stable_ducklake_version }}", config["stable_ducklake_version"])
    return doc_body
...
def adjust_links_in_doc_body(doc_body):
    ...
    doc_body = re.sub(
            r"\[installation page\]\((.*)?\)",
            r"[installation page](https://ducklake.select/docs/installation/)",
            doc_body
        )
...
def change_links(doc_body):
    ...
    for match in matches:
        ...
        if original_link.startswith("/"):
            full_url_link = f"https://ducklake.select{original_link}"
        ...
        if original_link.startswith("{% post_url "):
            new_link = re.sub(
                ...
                r"https://ducklake.select/\1/\2/\3/\4",
                original_link
            )
